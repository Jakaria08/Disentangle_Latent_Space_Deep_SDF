...
import reconstruct
from networks.longitudinal_flow_64_160_32_personal_age import build_temporal_flow


def binary_classification_metrics(targets, probabilities, threshold=0.5):
    targets = np.asarray(targets, dtype=np.int64).reshape(-1)
    probabilities = np.asarray(probabilities, dtype=np.float64).reshape(-1)
    if targets.size == 0 or probabilities.size != targets.size:
        return {}
    ...
    pos = probabilities[targets == 1]
    neg = probabilities[targets == 0]
    ...
    bce = float(
        -np.mean(targets * np.log(clipped) + (1 - targets) * np.log(1.0 - clipped))
    )
    return {
        "bce": bce,
        "accuracy": accuracy,
...
def write_diagnosis_predictions(path, rows):
    if not rows:
        return
    ...
    fieldnames = [
        ...
        "anchor_norm",
        "condition_mode",
        "forecast_mean_chamfer",
        "forecast_final_chamfer",
        "forecast_scan_count",
    ]
...
def predict_anchor_disease(temporal_flow, z, threshold=0.5):
    flow = temporal_flow.module if hasattr(temporal_flow, "module") else temporal_flow
    if not callable(getattr(flow, "anchor_disease_probability", None)):
        raise RuntimeError(
            "The configured temporal flow has no anchor disease classifier."
        )
    with torch.no_grad():
        probability = flow.anchor_disease_probability(z).view(-1, 1)
    hard = (probability >= float(threshold)).to(dtype=z.dtype)
    return probability.to(dtype=z.dtype), hard


def anchor_disease_probability(temporal_flow, z, detach_latent=True):
    flow = temporal_flow.module if hasattr(temporal_flow, "module") else temporal_flow
    if not callable(getattr(flow, "anchor_disease_probability", None)):
        raise RuntimeError(
            "The configured temporal flow has no anchor disease classifier."
        )
    classifier_z = z.detach() if detach_latent else z
    return flow.anchor_disease_probability(classifier_z).view(-1, 1)


def compose_flow_condition(
    temporal_flow,
    diagnosis,
    baseline_anchor,
    detach_anchor=True,
    ...
    flow = temporal_flow.module if hasattr(temporal_flow, "module") else temporal_flow
    compose = getattr(flow, "compose_condition", None)
    if callable(compose):
        return compose(
            diagnosis,
            baseline_anchor,
...
def disease_teacher_forcing_alpha(epoch, start_epoch, end_epoch):
    epoch = int(epoch)
    ...
    if end_epoch <= start_epoch or epoch >= end_epoch:
        return 0.0
    return 1.0 - (epoch - start_epoch) / float(end_epoch - start_epoch)


def training_disease_gate(
    temporal_flow,
    anchors,
    diagnosis_target,
    epoch,
    mode,
    teacher_forcing_start_epoch,
    teacher_forcing_end_epoch,
    detach_latent=True,
    threshold=0.5,
):
    mode = str(mode).strip().lower()
    diagnosis_target = diagnosis_target.view(-1, 1).to(
        device=anchors.device, dtype=anchors.dtype
    )
    predicted = anchor_disease_probability(
        temporal_flow,
        anchors,
        detach_latent=detach_latent,
    )
    if mode == "oracle":
        return diagnosis_target, predicted, 1.0
    if mode == "predicted_soft":
        return predicted, predicted, 0.0
    if mode == "predicted_hard":
        return (predicted >= float(threshold)).to(anchors.dtype), predicted, 0.0
    if mode != "scheduled_predicted_soft":
        raise ValueError(f"Unknown TrainDiseaseConditionMode='{mode}'.")
    alpha = disease_teacher_forcing_alpha(
        epoch,
        teacher_forcing_start_epoch,
        teacher_forcing_end_epoch,
    )
    gate = alpha * diagnosis_target + (1.0 - alpha) * predicted
    return gate, predicted, alpha
...
def compute_disentangled_velocity_losses(
    ...
    use_anchor_disease_classification=False,
    anchor_disease_classification_lambda=0.0,
    anchor_disease_classifier_detach_latent=True,
    anchor_disease_pos_weight=1.0,
    diagnosis_target=None,
    use_age_modulation_regularization=False,
    age_modulation_magnitude_lambda=0.0,
    age_modulation_center_lambda=0.0,
):
    ...
    out = {
        ...
        "anchor_disease_classification": zero,
        "age_modulation_magnitude": zero,
        "age_modulation_center": zero,
        "age_adversarial_bce": zero,
        "residual_adversarial_bce": zero,
        "disease_classification_bce": zero,
        "anchor_disease_classification_bce": zero,
        "age_adversarial_acc": zero,
        "residual_adversarial_acc": zero,
        "disease_classification_acc": zero,
        "anchor_disease_classification_acc": zero,
        "anchor_disease_probabilities": z.new_empty((0,)),
        "anchor_disease_targets": z.new_empty((0,)),
        "age_norm": zero,
        "age_population_norm": zero,
        "age_individual_norm": zero,
        "age_modulation_mean": zero,
        "age_modulation_std": zero,
        "age_modulation_deviation_norm": zero,
    ...
    diagnosis_gate = components["diagnosis"].view(-1)
    diagnosis = (
        diagnosis_gate
        if diagnosis_target is None
        else diagnosis_target.to(device=z.device, dtype=z.dtype).view(-1)
    )
    v_age = components["age"]
    v_age_population = components.get("age_population", v_age)
    v_age_individual = components.get(
        "age_individual",
        torch.zeros_like(v_age),
    )
    ...
    out["age_population_norm"] = torch.mean(
        torch.norm(v_age_population, dim=1)
    ).detach()
    out["age_individual_norm"] = torch.mean(
        torch.norm(v_age_individual, dim=1)
    ).detach()
    ...
    modulation = components.get("age_modulation", None)
    if modulation is not None:
        deviation = modulation - 1.0
        out["age_modulation_mean"] = modulation.mean().detach()
        out["age_modulation_std"] = modulation.std(unbiased=False).detach()
        out["age_modulation_deviation_norm"] = torch.mean(
            torch.norm(deviation, dim=1)
        ).detach()
        if use_age_modulation_regularization:
            if float(age_modulation_magnitude_lambda) > 0.0:
                magnitude = float(age_modulation_magnitude_lambda) * torch.mean(
                    deviation.pow(2)
                )
                out["age_modulation_magnitude"] = magnitude
                out["loss"] = out["loss"] + magnitude
            if float(age_modulation_center_lambda) > 0.0:
                center = float(age_modulation_center_lambda) * torch.mean(
                    torch.mean(modulation, dim=0).sub(1.0).pow(2)
                )
                out["age_modulation_center"] = center
                out["loss"] = out["loss"] + center
    ...
    if (
        use_anchor_disease_classification
        and float(anchor_disease_classification_lambda) > 0.0
        and callable(getattr(flow, "anchor_disease_logits", None))
    ):
        classifier_z = z.detach() if anchor_disease_classifier_detach_latent else z
        classifier_target = target
        if anchor_disease_classifier_detach_latent and classifier_z.shape[0] > 1:
            classifier_z, inverse = torch.unique(
                classifier_z,
                dim=0,
                return_inverse=True,
            )
            first_indices = []
            for unique_idx in range(classifier_z.shape[0]):
                first_indices.append(
                    int(torch.nonzero(inverse == unique_idx, as_tuple=False)[0].item())
                )
            ...
            classifier_target = target.index_select(0, first_indices)
        logits = flow.anchor_disease_logits(classifier_z)
        pos_weight = z.new_tensor(float(anchor_disease_pos_weight))
        raw_bce = torch.nn.functional.binary_cross_entropy_with_logits(
            logits,
            classifier_target,
            pos_weight=pos_weight,
        )
        weighted_bce = float(anchor_disease_classification_lambda) * raw_bce
        probabilities = torch.sigmoid(logits.detach()).view(-1)
        out["anchor_disease_classification"] = weighted_bce
        out["anchor_disease_classification_bce"] = raw_bce.detach()
        out["anchor_disease_classification_acc"] = (
            (
                (probabilities >= 0.5).float()
                == classifier_target.view(-1)
            )
            .float()
            .mean()
        )
        out["anchor_disease_probabilities"] = probabilities
        out["anchor_disease_targets"] = classifier_target.view(-1).detach()
        out["loss"] = out["loss"] + weighted_bce
...
def _compute_real_scan_pair_batch_losses(
    ...
    for subject_idx in unique_subjects:
        ...
        subject_anchor_for_future = subject_anchor.detach()
        ...
        for scan_a, scan_b in scan_pairs:
            ...
            if use_age_conditioning:
                source_diagnosis = scan_age_condition[forward_source_idx].view(
                    1, -1
                )[:, 0:1]
                target_diagnosis = scan_age_condition[forward_target_idx].view(
                    1, -1
                )[:, 0:1]
                source_condition = compose_flow_condition(
                    temporal_flow,
                    source_diagnosis,
                    subject_anchor_for_future,
                    detach_anchor=True,
                )
                target_condition = compose_flow_condition(
                    temporal_flow,
                    target_diagnosis,
                    subject_anchor_for_future,
                    detach_anchor=True,
...
def initialize_subject_anchors_from_pretrained_scan_latents(
    ...
    mode="mean_all_scans",
    scan_to_time_cpu=None,
):
    mode = str(mode).strip().lower()
    if mode not in ("mean_all_scans", "baseline_scan"):
        raise ValueError(
            "PretrainedSubjectAnchorInitMode must be 'mean_all_scans' or "
            f"'baseline_scan', got '{mode}'."
        )
    if mode == "baseline_scan" and scan_to_time_cpu is None:
        raise ValueError("baseline_scan initialization requires scan_to_time_cpu.")
    ...
    baseline_times = torch.full((num_subjects,), float("inf"), dtype=torch.float32)
    ...
    for scan_idx, fpath in enumerate(npyfiles):
        ...
        if mode == "baseline_scan":
            scan_time = float(scan_to_time_cpu[scan_idx].item())
            if scan_time < float(baseline_times[subj_idx].item()):
                baseline_times[subj_idx] = scan_time
    ...
    if expected_scans_per_subject is not None and mode == "mean_all_scans":
    ...
    try:
        ...
        def _condition_from_sampled_time(tt):
            if not use_age_conditioning:
                return None
            if disease_condition_mode != "oracle":
                flow = (
                    temporal_flow.module
                    if hasattr(temporal_flow, "module")
                    else temporal_flow
                )
                condition = flow.anchor_disease_probability(anchor).view(1, -1)
                if disease_condition_mode == "predicted_hard":
                    condition = (
                        condition >= float(disease_threshold)
                    ).to(anchor.dtype)
                condition = compose_flow_condition(
                    temporal_flow,
                    condition,
                    anchor,
                    detach_anchor=False,
                )
                return condition.repeat(tt.shape[0], 1)
            if condition_fit is not None:
                ...
                diagnosis = intercept.unsqueeze(0) + tt_clamped * slope.unsqueeze(0)
                repeated_anchor = anchor.expand(tt.shape[0], -1)
                return compose_flow_condition(
                    temporal_flow,
                    diagnosis,
                    repeated_anchor,
                    detach_anchor=False,
            ...
            if condition_fallback is not None:
                ...
                diagnosis = fallback.repeat(tt.shape[0], 1)
                repeated_anchor = anchor.expand(tt.shape[0], -1)
                return compose_flow_condition(
                    temporal_flow,
                    diagnosis,
        ...
        for _ in range(int(num_iterations)):
            ...
            if use_age_conditioning and disease_condition_mode != "oracle":
                flow = (
                    temporal_flow.module
                    if hasattr(temporal_flow, "module")
                    else temporal_flow
                )
                optimized_condition = flow.anchor_disease_probability(anchor).view(
                    1, -1
                )
                if disease_condition_mode == "predicted_hard":
                    optimized_condition = (
                        optimized_condition >= float(disease_threshold)
                    ).to(anchor.dtype)
                optimized_condition = compose_flow_condition(
                    temporal_flow,
                    optimized_condition,
                    anchor,
                    detach_anchor=False,
            ...
            for obs in observations:
                ...
                age_cond = None
                if use_age_conditioning:
                    if optimized_condition is not None:
                        age_cond = optimized_condition.repeat(xyz.shape[0], 1)
                    else:
                        obs_cond = _condition_to_1d_tensor(obs.get("age_cond", None))
                        if obs_cond is not None:
                            diagnosis = obs_cond.to(
                                device=device, dtype=anchor.dtype
                            ).view(1, -1)
                            age_cond = compose_flow_condition(
                                temporal_flow,
                                diagnosis,
                                anchor,
                                detach_anchor=False,
...
def main_function(experiment_directory: str, continue_from, batch_split: int, gpu: int = None):
    ...
    use_anchor_disease_classification = bool(
        get_spec_with_default(specs, "UseAnchorDiseaseClassification", False)
    )
    anchor_disease_classification_lambda = float(
        get_spec_with_default(specs, "AnchorDiseaseClassificationLambda", 0.0)
    )
    anchor_disease_classifier_detach_latent = bool(
        get_spec_with_default(specs, "AnchorDiseaseClassifierDetachLatent", True)
    )
    anchor_disease_threshold = float(
        get_spec_with_default(specs, "AnchorDiseaseThreshold", 0.5)
    )
    age_personalization_mode = str(
        get_spec_with_default(specs, "AgePersonalizationMode", "population")
    ).strip().lower()
    if age_personalization_mode not in ("population", "baseline_modulation"):
        raise RuntimeError(
            "AgePersonalizationMode must be 'population' or "
            "'baseline_modulation'."
        )
    use_age_modulation_regularization = bool(
        get_spec_with_default(
            specs,
            "UseAgeModulationRegularization",
            age_personalization_mode == "baseline_modulation",
        )
    )
    age_modulation_magnitude_lambda = float(
        get_spec_with_default(specs, "AgeModulationMagnitudeLambda", 0.0)
    )
    age_modulation_center_lambda = float(
        get_spec_with_default(specs, "AgeModulationCenterLambda", 0.0)
    )
    anchor_training_mode = str(
        get_spec_with_default(specs, "SubjectAnchorTrainingMode", "all_scans")
    ).strip().lower()
    if anchor_training_mode not in ("all_scans", "baseline_only"):
        raise RuntimeError(
            "SubjectAnchorTrainingMode must be 'all_scans' or 'baseline_only'."
        )
    train_disease_condition_mode = str(
        get_spec_with_default(specs, "TrainDiseaseConditionMode", "oracle")
    ).strip().lower()
    valid_train_condition_modes = {
        "oracle",
        "predicted_soft",
        "predicted_hard",
        "scheduled_predicted_soft",
    }
    if train_disease_condition_mode not in valid_train_condition_modes:
        raise RuntimeError(
            "TrainDiseaseConditionMode must be one of "
            f"{sorted(valid_train_condition_modes)}."
        )
    disease_teacher_forcing_start_epoch = int(
        get_spec_with_default(specs, "DiseaseTeacherForcingStartEpoch", 100)
    )
    disease_teacher_forcing_end_epoch = int(
        get_spec_with_default(specs, "DiseaseTeacherForcingEndEpoch", 300)
    )
    logging.info(
    ...
    if use_disentangled_velocity_flow:
        ...
        logging.info(
            "Anchor diagnosis classifier: enabled=%s, lambda=%s, hidden_dims=%s, "
            "detach_latent=%s, threshold=%s",
            use_anchor_disease_classification,
            anchor_disease_classification_lambda,
            get_spec_with_default(
                specs, "AnchorDiseaseClassifierHiddenDims", [128, 64]
            ),
            anchor_disease_classifier_detach_latent,
            anchor_disease_threshold,
        )
        logging.info(
            "Anchor training mode=%s; train disease condition=%s; "
            "teacher forcing epochs=%d..%d",
            anchor_training_mode,
            train_disease_condition_mode,
            disease_teacher_forcing_start_epoch,
            disease_teacher_forcing_end_epoch,
        )
        logging.info(
            "Age personalization: mode=%s, hidden_dims=%s, amplitude=%s, "
            "regularization=%s (magnitude_lambda=%s, center_lambda=%s)",
            age_personalization_mode,
            get_spec_with_default(specs, "AgeModulationHiddenDims", [128, 64]),
            get_spec_with_default(specs, "AgeModulationAmplitude", 0.5),
            use_age_modulation_regularization,
            age_modulation_magnitude_lambda,
            age_modulation_center_lambda,
        )
    logging.info(
    ...
    pretrained_subject_anchor_init_mode = str(
        get_spec_with_default(
            specs,
            "PretrainedSubjectAnchorInitMode",
            "mean_all_scans",
        )
    ).strip().lower()
    if pretrained_subject_anchor_init_mode not in (
        "mean_all_scans",
        "baseline_scan",
        "random",
    ):
        raise RuntimeError(
            "PretrainedSubjectAnchorInitMode must be mean_all_scans, "
            "baseline_scan, or random."
        )
    ...
    train_subject_ids = sorted(set(longitudinal_meta["subject_ids"]))
    ...
    anchor_disease_pos_weight = 1.0
    if use_anchor_disease_classification:
        if subject_baseline_age is None or subject_baseline_age.shape[1] < 1:
            raise RuntimeError(
                "Anchor diagnosis classification requires diagnosis as the first "
                "configured condition."
            )
        subject_diagnosis = subject_baseline_age[:, 0]
        positive_count = int(torch.sum(subject_diagnosis >= 0.5).item())
        negative_count = int(torch.sum(subject_diagnosis < 0.5).item())
        anchor_disease_pos_weight = negative_count / max(1, positive_count)
        logging.info(
            "Anchor diagnosis class balance: healthy=%d, diseased=%d, pos_weight=%.6f",
            negative_count,
            positive_count,
            anchor_disease_pos_weight,
        )
    ...
    eval_disease_condition_mode = str(
        get_spec_with_default(specs, "EvalDiseaseConditionMode", "oracle")
    ).strip().lower()
    valid_disease_condition_modes = {"oracle", "predicted_soft", "predicted_hard"}
    if eval_disease_condition_mode not in valid_disease_condition_modes:
        raise RuntimeError(
            "EvalDiseaseConditionMode must be one of "
            f"{sorted(valid_disease_condition_modes)}, got "
            f"'{eval_disease_condition_mode}'."
        )
    if (
        eval_disease_condition_mode != "oracle"
        and not use_anchor_disease_classification
    ):
        raise RuntimeError(
            "Predicted test diagnosis gating requires "
            "UseAnchorDiseaseClassification=true."
        )
    ...
    logging.info(
        "Test disease condition mode=%s (threshold=%s)",
        eval_disease_condition_mode,
        anchor_disease_threshold,
    )
    ...
    if use_pretrained_subject_anchor_init:
        ...
        elif pretrained_subject_anchor_init_mode == "random":
            logging.info(
                "Pretrained subject-anchor initialization requested but mode=random; "
    ...
    try:
        ...
        for epoch in range(start_epoch, num_epochs + 1):
            ...
            epoch_age_modulation_magnitude_losses = []
            epoch_age_modulation_center_losses = []
            epoch_velocity_age_adversarial_bces = []
            epoch_velocity_residual_adversarial_bces = []
            epoch_velocity_disease_classification_bces = []
            epoch_anchor_disease_classification_losses = []
            epoch_anchor_disease_classification_bces = []
            epoch_anchor_disease_classification_accs = []
            epoch_anchor_disease_probabilities = []
            epoch_anchor_disease_targets = []
            ...
            epoch_velocity_age_population_norms = []
            epoch_velocity_age_individual_norms = []
            epoch_age_modulation_means = []
            epoch_age_modulation_stds = []
            epoch_age_modulation_deviation_norms = []
            ...
            for sdf_data, indices in sdf_loader:
                ...
                age_modulation_magnitude_loss_tb = 0.0
                age_modulation_center_loss_tb = 0.0
                velocity_age_adversarial_bce_tb = 0.0
                velocity_residual_adversarial_bce_tb = 0.0
                velocity_disease_classification_bce_tb = 0.0
                anchor_disease_classification_loss_tb = 0.0
                anchor_disease_classification_bce_tb = 0.0
                anchor_disease_classification_acc_tb = 0.0
                ...
                velocity_age_population_norm_tb = 0.0
                velocity_age_individual_norm_tb = 0.0
                age_modulation_mean_tb = 0.0
                age_modulation_std_tb = 0.0
                age_modulation_deviation_norm_tb = 0.0
                ...
                for i in range(batch_split):
                    ...
                    scan_ages = None
                    scan_diagnosis_targets = None
                    if use_age_conditioning:
                        scan_diagnosis_targets = scan_age_condition.index_select(
                            0, indices[i]
                        )[:, 0:1]
                        gate_subjects, gate_inverse = torch.unique(
                            scan_subject_indices,
                            return_inverse=True,
                        )
                        gate_anchors = lat_vecs(gate_subjects)
                        gate_targets = subject_baseline_age.index_select(
                            0, gate_subjects
                        )[:, 0:1]
                        subject_gates, _, teacher_forcing_alpha = training_disease_gate(
                            temporal_flow,
                            gate_anchors,
                            gate_targets,
                            epoch=epoch,
                            mode=train_disease_condition_mode,
                            teacher_forcing_start_epoch=(
                                disease_teacher_forcing_start_epoch
                            ),
                            teacher_forcing_end_epoch=(
                            ...
                            detach_latent=(
                                anchor_disease_classifier_detach_latent
                                or anchor_training_mode == "baseline_only"
                            ),
                            threshold=anchor_disease_threshold,
                        )
                        subject_conditions = compose_flow_condition(
                            temporal_flow,
                            subject_gates,
                            gate_anchors,
                            detach_anchor=True,
                        )
                        scan_ages = subject_conditions.index_select(0, gate_inverse)
                    if anchor_training_mode == "baseline_only":
                        baseline_mask = (
                            (scan_times - start_times).abs() <= 1e-7
                        ).expand_as(subject_anchor_batch)
                        subject_anchor_for_flow = torch.where(
                            baseline_mask,
                            subject_anchor_batch,
                            subject_anchor_batch.detach(),
                        )
                    else:
                        subject_anchor_for_flow = subject_anchor_batch
                    batch_vecs = apply_temporal_flow(
                        temporal_flow,
                        subject_anchor_for_flow,
                        start_times,
                        scan_times,
                        age_cond=scan_ages,
                    )
                    ...
                    if use_zero_displacement_loss and zero_displacement_lambda > 0.0:
                        ...
                        baseline_age = None
                        if use_age_conditioning:
                            baseline_diagnosis = subject_baseline_age.index_select(
                                0, unique_subjects
                            )[:, 0:1]
                            baseline_age = compose_flow_condition(
                                temporal_flow,
                                baseline_diagnosis,
                                unique_anchor_vecs,
                                detach_anchor=True,
                    ...
                    if use_disentangled_velocity_flow:
                        ...
                        if anchor_training_mode == "baseline_only":
                            unique_anchors_for_losses = unique_anchors.detach()
                        else:
                            unique_anchors_for_losses = unique_anchors
                        ...
                        unique_diagnosis_targets = None
                        if use_age_conditioning:
                            unique_diagnosis_targets = scan_age_condition.index_select(
                                0, unique_scan_indices
                            )[:, 0:1]
                            unique_scan_gates, _, _ = training_disease_gate(
                                temporal_flow,
                                unique_anchors,
                                unique_diagnosis_targets,
                                epoch=epoch,
                                mode=train_disease_condition_mode,
                                teacher_forcing_start_epoch=(
                                    disease_teacher_forcing_start_epoch
                            ...
                            unique_scan_ages = compose_flow_condition(
                                temporal_flow,
                                unique_scan_gates,
                                unique_anchors,
                                detach_anchor=True,
                        ...
                        velocity_losses = compute_disentangled_velocity_losses(
                            ...
                            anchor_disease_pos_weight=anchor_disease_pos_weight,
                            diagnosis_target=unique_diagnosis_targets,
                            use_age_modulation_regularization=(
                                use_age_modulation_regularization
                            ),
                            age_modulation_magnitude_lambda=(
                        ...
                        age_modulation_magnitude_loss_tb += float(
                            velocity_losses[
                                "age_modulation_magnitude"
                            ].detach().item()
                        )
                        age_modulation_center_loss_tb += float(
                            velocity_losses["age_modulation_center"].detach().item()
                        )
                        anchor_disease_classification_loss_tb += float(
                            velocity_losses[
                                "anchor_disease_classification"
                            ].detach().item()
                        )
                        ...
                        anchor_disease_classification_bce_tb += float(
                            velocity_losses[
                                "anchor_disease_classification_bce"
                            ].detach().item()
                        )
                        ...
                        anchor_disease_classification_acc_tb += float(
                            velocity_losses[
                                "anchor_disease_classification_acc"
                            ].detach().item()
                        )
                        if (
                            velocity_losses[
                                "anchor_disease_probabilities"
                            ].numel()
                            > 0
                        ):
                            epoch_anchor_disease_probabilities.extend(
                                velocity_losses[
                                    "anchor_disease_probabilities"
                                ].detach().cpu().tolist()
                            )
                            epoch_anchor_disease_targets.extend(
                                velocity_losses[
                                    "anchor_disease_targets"
                                ].detach().cpu().tolist()
                            )
                        velocity_age_norm_tb += float(
                            velocity_losses["age_norm"].detach().item()
                        )
                        velocity_age_population_norm_tb += float(
                            velocity_losses[
                                "age_population_norm"
                            ].detach().item()
                        )
                        velocity_age_individual_norm_tb += float(
                            velocity_losses[
                                "age_individual_norm"
                            ].detach().item()
                        )
                        age_modulation_mean_tb += float(
                            velocity_losses[
                                "age_modulation_mean"
                            ].detach().item()
                        )
                        age_modulation_std_tb += float(
                            velocity_losses[
                                "age_modulation_std"
                            ].detach().item()
                        )
                        age_modulation_deviation_norm_tb += float(
                            velocity_losses[
                                "age_modulation_deviation_norm"
                            ].detach().item()
                        )
                    ...
                    if (
                        ...
                        if unique_subjects.numel() > 0:
                            ...
                            if anchor_training_mode == "baseline_only":
                                subject_anchors = subject_anchors.detach()
                            ...
                            if use_age_conditioning:
                                consistency_targets = subject_baseline_age.index_select(
                                    0, unique_subjects
                                )[:, 0:1]
                                consistency_gate, _, _ = training_disease_gate(
                                    temporal_flow,
                                    subject_anchors,
                                    consistency_targets,
                                    epoch=epoch,
                                ...
                                consistency_condition = compose_flow_condition(
                                    temporal_flow,
                                    consistency_gate,
                                    subject_anchors,
                                    detach_anchor=True,
                ...
                epoch_age_modulation_magnitude_losses.append(
                    age_modulation_magnitude_loss_tb
                )
                epoch_age_modulation_center_losses.append(
                    age_modulation_center_loss_tb
                )
                epoch_anchor_disease_classification_losses.append(
                    anchor_disease_classification_loss_tb
                )
                ...
                epoch_anchor_disease_classification_bces.append(
                    anchor_disease_classification_bce_tb
                )
                ...
                epoch_anchor_disease_classification_accs.append(
                    anchor_disease_classification_acc_tb
                )
                epoch_velocity_age_norms.append(velocity_age_norm_tb)
                epoch_velocity_age_population_norms.append(
                    velocity_age_population_norm_tb
                )
                epoch_velocity_age_individual_norms.append(
                    velocity_age_individual_norm_tb
                )
                epoch_age_modulation_means.append(age_modulation_mean_tb)
                epoch_age_modulation_stds.append(age_modulation_std_tb)
                epoch_age_modulation_deviation_norms.append(
                    age_modulation_deviation_norm_tb
                )
            ...
            epoch_age_modulation_magnitude_loss = (
                sum(epoch_age_modulation_magnitude_losses)
                / len(epoch_age_modulation_magnitude_losses)
            )
            epoch_age_modulation_center_loss = (
                sum(epoch_age_modulation_center_losses)
                / len(epoch_age_modulation_center_losses)
            )
            epoch_anchor_disease_classification_loss = (
                sum(epoch_anchor_disease_classification_losses)
                / len(epoch_anchor_disease_classification_losses)
            )
            ...
            epoch_anchor_disease_classification_bce = (
                sum(epoch_anchor_disease_classification_bces)
                / len(epoch_anchor_disease_classification_bces)
            )
            ...
            epoch_anchor_disease_classification_acc = (
                sum(epoch_anchor_disease_classification_accs)
                / len(epoch_anchor_disease_classification_accs)
            )
            epoch_anchor_disease_metrics = binary_classification_metrics(
                epoch_anchor_disease_targets,
                epoch_anchor_disease_probabilities,
                threshold=anchor_disease_threshold,
            )
            if use_anchor_disease_classification:
                ...
                with torch.no_grad():
                    all_train_probabilities = (
                        flow_module.anchor_disease_probability(lat_vecs.weight)
                        .view(-1)
                        .detach()
                        .cpu()
                        .numpy()
                ...
                all_train_targets = (
                    subject_baseline_age[:, 0].detach().cpu().numpy()
                )
                epoch_anchor_disease_metrics = binary_classification_metrics(
                    all_train_targets,
                    all_train_probabilities,
                ...
                if eval_test_frequency > 0 and epoch % eval_test_frequency == 0:
                    train_diagnosis_rows = []
                    for subject_idx in range(num_subjects):
                        probability = float(all_train_probabilities[subject_idx])
                        train_diagnosis_rows.append(
                            {
                                "epoch": int(epoch),
                                "split": "train_all_visit_anchor",
                                "subject_id": str(train_subject_ids[subject_idx]),
                                "true_diagnosis": int(
                                    all_train_targets[subject_idx] >= 0.5
                                ),
                                "predicted_probability": probability,
                                "predicted_class": int(
                                    probability >= anchor_disease_threshold
                                ),
                                "anchor_norm": float(
                                    lat_vecs.weight[subject_idx].norm().item()
                                ),
                                "condition_mode": "all_visit_anchor",
                            }
                        )
                    write_diagnosis_predictions(
                        os.path.join(
                            experiment_directory,
                            "DiagnosisPredictions",
                            f"train_epoch_{epoch}.csv",
                        ),
                        train_diagnosis_rows,
                    )
            ...
            epoch_velocity_age_population_norm = (
                sum(epoch_velocity_age_population_norms)
                / len(epoch_velocity_age_population_norms)
            )
            epoch_velocity_age_individual_norm = (
                sum(epoch_velocity_age_individual_norms)
                / len(epoch_velocity_age_individual_norms)
            )
            epoch_age_modulation_mean = (
                sum(epoch_age_modulation_means)
                / len(epoch_age_modulation_means)
            )
            epoch_age_modulation_std = (
                sum(epoch_age_modulation_stds)
                / len(epoch_age_modulation_stds)
            )
            epoch_age_modulation_deviation_norm = (
                sum(epoch_age_modulation_deviation_norms)
                / len(epoch_age_modulation_deviation_norms)
            )
            ...
            if use_disentangled_velocity_flow:
                ...
                if use_anchor_disease_classification:
                    print(
                        f"Epoch {epoch} anchor disease classification loss "
                        f"(weighted): {epoch_anchor_disease_classification_loss}"
                    )
                    print(
                        f"Epoch {epoch} anchor disease classifier BCE/acc: "
                        f"{epoch_anchor_disease_classification_bce}/"
                        f"{epoch_anchor_disease_classification_acc}"
                    )
                    if epoch_anchor_disease_metrics:
                        print(
                            f"Epoch {epoch} anchor disease classifier "
                            f"balanced_acc/AUC: "
                            f"{epoch_anchor_disease_metrics['balanced_accuracy']}/"
                            f"{epoch_anchor_disease_metrics['auc']}"
                        )
                print(f"Epoch {epoch} velocity age norm: {epoch_velocity_age_norm}")
                print(
                    f"Epoch {epoch} population/individual age norms: "
                    f"{epoch_velocity_age_population_norm}/"
                    f"{epoch_velocity_age_individual_norm}"
                )
                if age_personalization_mode == "baseline_modulation":
                    print(
                        f"Epoch {epoch} age modulation mean/std/deviation norm: "
                        f"{epoch_age_modulation_mean}/"
                        f"{epoch_age_modulation_std}/"
                        f"{epoch_age_modulation_deviation_norm}"
                    )
                    print(
                        f"Epoch {epoch} age modulation magnitude/center losses: "
                        f"{epoch_age_modulation_magnitude_loss}/"
                        f"{epoch_age_modulation_center_loss}"
                    )
                print(
            ...
            current_teacher_forcing_alpha = disease_teacher_forcing_alpha(
                epoch,
                disease_teacher_forcing_start_epoch,
                disease_teacher_forcing_end_epoch,
            )
            summary_writer.add_scalar(
                "Diagnosis/teacher_forcing_alpha",
                current_teacher_forcing_alpha,
                global_step=epoch,
            )
            ...
            if use_disentangled_velocity_flow:
                ...
                summary_writer.add_scalar(
                    "VelocityNorm/age_population",
                    epoch_velocity_age_population_norm,
                    global_step=epoch,
                )
                summary_writer.add_scalar(
                    "VelocityNorm/age_individual",
                    epoch_velocity_age_individual_norm,
                    global_step=epoch,
                )
                summary_writer.add_scalar(
                ...
                summary_writer.add_scalar(
                    "AgeModulation/deviation_norm",
                    epoch_age_modulation_deviation_norm,
                    global_step=epoch,
                )
                summary_writer.add_scalar(
                    "Loss/train_age_modulation_magnitude",
                    epoch_age_modulation_magnitude_loss,
                    global_step=epoch,
                )
                summary_writer.add_scalar(
                    "Loss/train_age_modulation_center",
                    epoch_age_modulation_center_loss,
                    global_step=epoch,
                )
                summary_writer.add_scalar(
                ...
                if use_anchor_disease_classification:
                    summary_writer.add_scalar(
                        "Diagnosis/train_loss_weighted",
                        epoch_anchor_disease_classification_loss,
                        global_step=epoch,
                    )
                    summary_writer.add_scalar(
                        "Diagnosis/train_batch_bce",
                        epoch_anchor_disease_classification_bce,
                        global_step=epoch,
                    )
                    if epoch_anchor_disease_metrics:
                        for metric_name, metric_value in epoch_anchor_disease_metrics.items():
                            if np.isfinite(metric_value):
                                summary_writer.add_scalar(
                                    f"Diagnosis/train_{metric_name}",
                                    metric_value,
                                    global_step=epoch,
                                )
            if use_gmm_prior:
            ...
            if torus_path is not None:
                ...
                if epoch % eval_train_frequency == 0:
                    ...
                    for index in eval_train_scene_idxs:
                        ...
                        eval_age = None
                        if use_age_conditioning:
                            eval_target = subject_baseline_age.index_select(
                                0, subject_idx
                            )[:, 0:1]
                            eval_gate, _, _ = training_disease_gate(
                                temporal_flow,
                                subject_anchor,
                                eval_target,
                                epoch=epoch,
                                ...
                                detach_latent=anchor_disease_classifier_detach_latent,
                                threshold=anchor_disease_threshold,
                            )
                            eval_age = compose_flow_condition(
                                temporal_flow,
                                eval_gate,
                                subject_anchor,
                                detach_anchor=True,
                ...
                if epoch % eval_test_frequency == 0:
                    ...
                    test_latents = []
                    subject_loss_labels = []
                    test_diagnosis_targets = []
                    test_diagnosis_probabilities = []
                    test_diagnosis_rows = []
                    for sid in eval_test_subject_ids:
                        ...
                        for scan_idx in observed_indices:
                            ...
                            if (
                                use_age_conditioning
                                and eval_disease_condition_mode == "oracle"
                            ):
                                obs_item["age_cond"] = (
                        ...
                        subject_anchor, anchor_loss_hist = (
                            optimize_subject_anchor_from_observations(
                                ...
                                use_age_conditioning=use_age_conditioning,
                                disease_condition_mode=eval_disease_condition_mode,
                                disease_threshold=anchor_disease_threshold,
                            )
                        )
                        logging.debug(
                        ...
                        if use_age_conditioning:
                            true_diagnosis = float(
                                test_scan_age_condition[
                            ...
                            true_diagnosis = 0.0
                        if use_anchor_disease_classification:
                            predicted_soft, predicted_hard = predict_anchor_disease(
                                temporal_flow,
                                subject_anchor,
                                threshold=anchor_disease_threshold,
                            )
                            predicted_probability = float(predicted_soft.item())
                            predicted_class = int(predicted_hard.item())
                            test_diagnosis_targets.append(int(true_diagnosis >= 0.5))
                            test_diagnosis_probabilities.append(predicted_probability)
                            test_diagnosis_rows.append(
                                {
                                    "epoch": int(epoch),
                                    "split": "test_baseline_anchor",
                                    "subject_id": str(sid),
                                    "true_diagnosis": int(true_diagnosis >= 0.5),
                                    "predicted_probability": predicted_probability,
                                    "predicted_class": predicted_class,
                                    "anchor_norm": float(subject_anchor.norm().item()),
                                    "condition_mode": eval_disease_condition_mode,
                                    "forecast_mean_chamfer": "",
                                    "forecast_final_chamfer": "",
                                    "forecast_scan_count": 0,
                                }
                            )
                        else:
                            predicted_soft = subject_anchor.new_tensor(
                                [[true_diagnosis]]
                            )
                            predicted_hard = predicted_soft
                        subject_forecast_chamfers = []
                        if eval_disease_condition_mode == "oracle":
                            eval_disease_gate = subject_anchor.new_tensor(
                                [[true_diagnosis]]
                            )
                        elif eval_disease_condition_mode == "predicted_hard":
                            eval_disease_gate = predicted_hard
                        else:
                            eval_disease_gate = predicted_soft
                        eval_disease_condition = compose_flow_condition(
                            temporal_flow,
                            eval_disease_gate,
                            subject_anchor,
                            detach_anchor=True,
                        ...
                        for scan_idx in target_indices:
                            ...
                            if test_mesh is not None:
                                ...
                                chamfer_dists.append(cd)
                                chamfer_dists_all.append(cd_all)
                                subject_forecast_chamfers.append(float(cd))
                                predicted_scan_count += 1

                        if use_anchor_disease_classification and subject_forecast_chamfers:
                            test_diagnosis_rows[-1]["forecast_mean_chamfer"] = float(
                                np.mean(subject_forecast_chamfers)
                            )
                            test_diagnosis_rows[-1]["forecast_final_chamfer"] = float(
                                subject_forecast_chamfers[-1]
                            )
                            test_diagnosis_rows[-1]["forecast_scan_count"] = len(
                                subject_forecast_chamfers
                            )

                    if use_anchor_disease_classification:
                        test_diagnosis_metrics = binary_classification_metrics(
                            test_diagnosis_targets,
                            test_diagnosis_probabilities,
                            threshold=anchor_disease_threshold,
                        )
                        for metric_name, metric_value in test_diagnosis_metrics.items():
                            if np.isfinite(metric_value):
                                summary_writer.add_scalar(
                                    f"Diagnosis/test_{metric_name}",
                                    metric_value,
                                    global_step=epoch,
                                )
                        ...
                        logging.info(
                            "Test anchor diagnosis metrics at epoch %d: %s",
                            epoch,
                            test_diagnosis_metrics,
                        )
