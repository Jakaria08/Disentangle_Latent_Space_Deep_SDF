...
def compute_additive_velocity_losses(
    ...
    if use_orthogonality and float(orthogonality_lambda) > 0.0:
        orth = (
            _safe_cosine_square(v_age, v_disease)
            + _safe_cosine_square(v_age, v_residual)
            + _safe_cosine_square(v_disease, v_residual)
        ) / 3.0
    ...
    if use_adversarial_leakage and float(adversarial_lambda) > 0.0:
        ...
        adv_age_from_residual = torch.mean(
            (flow.age_leak_from_residual(gradient_reverse(v_residual, grl)) - delta_age)
            ** 2
        )
        ...
        adv_disease_from_residual = _binary_cross_entropy_logits(
            flow.disease_leak_from_residual(gradient_reverse(v_residual, grl)),
            disease_score,
        )
        adv = (
            adv_age_from_disease
            + adv_age_from_residual
            + adv_disease_from_age
            + adv_disease_from_residual
        ) / 4.0
