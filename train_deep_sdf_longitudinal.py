...
class TemporalFlowMLP(torch.nn.Module):
    ...
    def forward(self, z, s, t):
        ...
        if t.dim() == 1:
            t = t.unsqueeze(1)
        x = torch.cat([z, s, t], dim=1)
        return self.net(x)


def apply_temporal_flow(temporal_flow, z, s, t):
    ...
    try:
        ...
        for _ in range(int(num_iterations)):
            ...
            for obs in observations:
                ...
                z_t = apply_temporal_flow(temporal_flow, anchor_expanded, s, t)
...
def main_function(experiment_directory: str, continue_from, batch_split: int):
    ...
    temporal_flow = TemporalFlowMLP(latent_size, flow_hidden_dims).to(train_device)
        ...
        for epoch in range(start_epoch, num_epochs + 1):
            ...
            for sdf_data, indices in sdf_loader:
                ...
                for i in range(batch_split):
                    ...
                    start_times = torch.zeros_like(scan_times)
            ...
            if torus_path is not None:
                ...
                if epoch % eval_train_frequency == 0:
                    ...
                    for index in eval_train_scene_idxs:
                        ...
                        eval_start = torch.zeros_like(eval_time)
                        subject_anchor = lat_vecs(subject_idx)
                        lat_vec = apply_temporal_flow(
                            temporal_flow, subject_anchor, eval_start, eval_time
                        )
                ...
                if epoch % eval_test_frequency == 0:
                    ...
                    for sid in eval_test_subject_ids:
                        ...
                        for scan_idx in observed_indices:
                            ...
                            observations.append(
                                {"samples": test_sdf_samples, "time": t_obs}
                            )
