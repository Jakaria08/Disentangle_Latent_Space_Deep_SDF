...
class DisentangledTemporalFlowMLP(torch.nn.Module):
    """Two-time latent velocity with fixed age, disease-gated, and residual blocks."""
    ...
    def __init__(
        ...
        if self.age_dim != 16:
            raise ValueError(
                "The initial poly/Fourier age embedding is implemented for "
                f"VelocityAgeDim=16, got {self.age_dim}."
            )
