...
class AdditiveNoResidualVelocityFlow(torch.nn.Module):
    """Full-latent additive velocity without residual: age + disease."""
    ...
    def velocity_components(self, z, s, t, age_cond=None):
        ...
        v_residual = torch.zeros_like(v_age)
        velocity = v_age + v_disease
        return {
            "velocity": velocity,
            "age": v_age,
            "disease": v_disease,
...
def build_temporal_flow(specs, latent_size, hidden_dims, age_condition_dim=0):
    ...
    return AdditiveNoResidualVelocityFlow(
