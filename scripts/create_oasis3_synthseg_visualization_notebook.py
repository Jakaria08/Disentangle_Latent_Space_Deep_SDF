#!/usr/bin/env python3
"""Create an inspection notebook for the OASIS-3 SynthSeg segmentations.

This follows the AIBL notebook (``scripts/create_aibl_synthseg_visualization_notebook.py``)
and the original ADNI one, with two OASIS-specific differences:

*   OASIS-3 tars carry only a raw NIfTI, with no FreeSurfer output, so there is no
    ``aseg.mgz`` to cross-validate against.  That section is replaced by a
    cross-cohort geometry and volume comparison against ADNI and AIBL, which is the
    closest available substitute and also verifies that ``mri_convert --conform``
    put OASIS on the same grid as the other two cohorts.
*   OASIS visit codes are ``V1``/``V3``/``V6``..., which carry no month offset, so the
    longitudinal section orders visits by index and reports the missing time axis
    rather than silently producing NaN months.
"""
...
DEFAULT_DATA_ROOT = REPO_ROOT / "data" / "OASIS_3_long" / "oasis3_synthseg"
DEFAULT_NOTEBOOK_PATH = DEFAULT_DATA_ROOT / "visualize_oasis3_synthseg_segmentation.ipynb"
DEFAULT_PRIMARY_SCAN = "OAS30001_V1"
...
def build_notebook(data_root: Path, primary_scan: str, adni_seg: Path, aibl_seg_dir: Path):
    ...
    cells.append(
        markdown(
            f"""# OASIS-3 SynthSeg Segmentation Inspection

This notebook inspects the OASIS-3 SynthSeg label maps produced by `data/oasis3_synthseg.sh`.
...
The mesh settings match the ADNI and AIBL notebooks, so statistics are directly
comparable across all three cohorts.

Two sections are specific to OASIS-3:

*   **Cross-cohort geometry check** - the OASIS-3 tars hold only a raw, defaced,
    oblique T1 NIfTI with no FreeSurfer output, so there is no `aseg.mgz` to
    cross-validate against as there is for AIBL.  Instead this verifies that
    `mri_convert --conform` placed OASIS on the same 256^3 1 mm LIA grid that
    ADNI and AIBL arrive on natively.
*   **Longitudinal consistency** - OASIS visit codes (`V1`, `V3`, `V6`, ...) carry no
    month offset, unlike ADNI/AIBL `bl`/`mNN`.  Visits are therefore ordered by index
    and the missing time axis is reported explicitly, because no rate of change can be
    computed until the real scan intervals are recovered.
...
# These match the ADNI and AIBL notebooks so mesh statistics stay comparable.
GAUSSIAN_SIGMA = 0.5
...
print(f"Found {{len(SEGMENTATION_PATHS)}} OASIS-3 SynthSeg volumes")
...
figure.update_layout(
    title=f"OASIS-3 {PRIMARY_SCAN}: SynthSeg hippocampus meshes, raw versus smoothed",
...
if ventricle_rows:
    ...
    ventricle_figure.update_layout(
        title=f"OASIS-3 {PRIMARY_SCAN}: SynthSeg lateral ventricle meshes, raw versus smoothed",
        template="plotly_white",
        width=1400,
        height=700,
        margin=dict(l=0, r=0, t=60, b=0),
...
'''
    ...
    cells.append(markdown("## Overview of every OASIS-3 segmentation"))
    cells.append(
        code(
...
'''
    ...
    cells.append(markdown("## Hippocampus across all OASIS-3 scans"))
    cells.append(
        code(
            '''segmentations_hipp_summary, segmentations_hipp_figure = build_multiscan_structure_figure(
    SEGMENTATION_PATHS,
    [17, 53],
    title="OASIS-3 SynthSeg hippocampus meshes across segmentations/",
...
'''
    ...
    cells.append(markdown("## Lateral ventricles across all OASIS-3 scans"))
    cells.append(
        code(
            '''segmentations_vent_summary, segmentations_vent_figure = build_multiscan_structure_figure(
    SEGMENTATION_PATHS,
    [4, 43],
    title="OASIS-3 SynthSeg lateral ventricle meshes across segmentations/",
...
OASIS-3 tars hold only a raw, defaced, oblique T1 NIfTI, so unlike AIBL there is no
FreeSurfer `aseg.mgz` to cross-validate against.  The substitute check is whether
`mri_convert --conform` (the `CONFORM_INPUT=1` step in `data/oasis3_synthseg.sh`)
really put OASIS on the grid that ADNI and AIBL arrive on natively.

What must match across cohorts:

*   shape `(256, 256, 256)`, voxel volume `1.0 mm^3`, orientation `LIA`
*   the affine's **3x3 direction matrix**

What will *not* match, and should not:

*   the affine's **translation column**.  `mri_convert --conform` preserves each scan's
    own scanner-space centre (`c_ras`), whereas the ADNI/AIBL `orig.mgz` files happen to
    carry `c_ras = 0`.  Meshes are built in world coordinates through the affine, so this
    is anatomically correct either way, and the ShapeWorks rigid stage removes translation
    regardless."""
        )
    )
    cells.append(
...
geometry_rows = [geometry_row(path, "OASIS-3") for path in SEGMENTATION_PATHS]
...
if reference_affine is None:
    ...
    print("No ADNI reference available; using the first OASIS scan as the reference instead.")
...
# Cohort composition differs by age and diagnosis, so a mean difference here is
# confounded and is NOT evidence of a preprocessing problem.
volume_rows = []
for row in geometry_rows:
    seg_path = (
        ADNI_REFERENCE_SEG
...
OASIS scan IDs are `OAS3XXXX_VN`, where `VN` is a **visit index, not a time offset**.
`viscode_to_month` in `scripts/adni_synthseg_minimal_correspondence.py` understands only
`bl` and `mNN`, so these codes yield `NaN` months and no rate of change can be computed
until the real scan intervals are recovered (OASIS-3 natively names sessions
`OAS3XXXX_MR_dNNNN`, where `dNNNN` is days from study entry).

Visits are therefore ordered by index here.  Direction is still checkable, and is the
useful signal: in an ageing or AD cohort hippocampal volume should trend down and
ventricular volume up across visits, with both hemispheres agreeing."""
        )
    )
    cells.append(
        code(
...
VISIT_RE = re.compile(r"^V(\\d+)$")


def parse_scan_id(scan_id: str) -> tuple[str, str, float]:
    subject_id, _, visit = scan_id.rpartition("_")
    match = VISIT_RE.match(visit)
    visit_index = float(match.group(1)) if match else float("nan")
    return subject_id, visit, visit_index


longitudinal_rows = []
for seg_path in SEGMENTATION_PATHS:
    scan_id = seg_path.parent.name
    subject_id, visit, visit_index = parse_scan_id(scan_id)
    seg_image = nib.load(str(seg_path))
    seg_data = np.asanyarray(seg_image.dataobj).astype(np.int16)
    voxel_volume = abs(float(np.linalg.det(seg_image.affine[:3, :3])))
...
longitudinal = pd.DataFrame(longitudinal_rows).sort_values(["subject_id", "visit_index"]).reset_index(drop=True)
display(longitudinal.round(1))

if longitudinal["visit_index"].isna().any():
    print("WARNING: some visit codes did not parse as V<n>; check the scan IDs above.")
print("NOTE: visit index is an ordering only. Months from baseline are unavailable,")
...
if repeat_subjects.empty:
    ...
    for _, row in change.iterrows():
        ...
        print(f"  {row['from']} -> {row['to']}: {verdict}")

    fig, axes = plt.subplots(1, 2, figsize=(14, 5))
    for subject_id, group in repeat_subjects.groupby("subject_id"):
        group = group.sort_values("visit_index")
        axes[0].plot(group["visit_index"], group["left_hippocampus_mm3"], marker="o", label=f"{subject_id} L")
        axes[0].plot(group["visit_index"], group["right_hippocampus_mm3"], marker="s", linestyle="--", label=f"{subject_id} R")
        axes[1].plot(group["visit_index"], group["left_lateral_ventricle_mm3"], marker="o", label=f"{subject_id} L")
        axes[1].plot(group["visit_index"], group["right_lateral_ventricle_mm3"], marker="s", linestyle="--", label=f"{subject_id} R")
...
def main() -> None:
    ...
    notebook = build_notebook(
        args.data_root.resolve(), args.primary_scan, args.adni_reference.resolve(), args.aibl_segmentations.resolve()
    )
