# Task 1: ADNI No-MCI Left Original Longitudinal Manifest

This folder is an isolated data-contract layer for the BrainODE comparison.
It reads existing ADNI metadata, original No-MCI split JSON files, original
meshes, and original SDF samples, then writes cleaned CSV/JSON artifacts for
later tasks.
...
- `examples/splits/splits_left_hippocampus_ADNI_No_MCI/*.json`
...
- `metadata/adni_no_mci_left_original_master.csv`
- `metadata/adni_no_mci_left_original_clean.csv`
- `metadata/adni_no_mci_left_original_subject_trajectories.json`
