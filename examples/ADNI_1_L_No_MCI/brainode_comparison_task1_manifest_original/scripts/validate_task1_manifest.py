#!/usr/bin/env python3
"""Validate Task 1 ADNI manifest artifacts."""
...
def validate_required_files(root: Path) -> Tuple[List[Path], List[Path]]:
    required = [
        root / "metadata/adni_no_mci_left_original_master.csv",
        root / "metadata/adni_no_mci_left_original_clean.csv",
        root / "metadata/adni_no_mci_left_original_subject_trajectories.json",
...
def main() -> None:
    ...
    master = read_csv(metadata_dir / "adni_no_mci_left_original_master.csv")
    clean = read_csv(metadata_dir / "adni_no_mci_left_original_clean.csv")
    pairs_all = read_csv(root / "pairs/pairs_all.csv")
    triplets_all = read_csv(root / "triplets/triplets_all.csv")
    trajectories = read_json(metadata_dir / "adni_no_mci_left_original_subject_trajectories.json")
    ...
    for row in master:
        if row["diagnosis"] not in {"CN", "AD"}:
            add_error(
                errors,
                "unexpected_master_diagnosis",
                "Master row contains a diagnosis other than CN or AD.",
                scan_id=row["scan_id"],
    ...
    for row in clean:
        ...
        if row["diagnosis"] not in {"CN", "AD"}:
            add_error(errors, "unexpected_clean_diagnosis", "Clean row contains non CN/AD diagnosis.", scan_id=row["scan_id"])
        if row["label_ad"] not in {"0", "1"}:
            add_error(errors, "invalid_label_ad", "Clean row label_ad is not 0 or 1.", scan_id=row["scan_id"], label=row["label_ad"])
