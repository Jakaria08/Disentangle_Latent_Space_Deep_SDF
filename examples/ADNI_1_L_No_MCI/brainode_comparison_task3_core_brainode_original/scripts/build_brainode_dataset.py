...
def main() -> int:
    ...
    for subject_id, subject_rows in sorted(
        ...
        for row in subject_rows:
            ...
            continuous_year = continuous_age_years(
                baseline_age_years, as_float(row["months_from_baseline"])
            )
            continuous_norm = normalize_age(
                continuous_year, age_min_train, age_max_train
            )
            visit_entry = {
                "scan_id": row["scan_id"],
