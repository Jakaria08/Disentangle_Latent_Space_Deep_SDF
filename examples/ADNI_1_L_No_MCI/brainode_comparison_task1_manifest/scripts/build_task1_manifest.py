#!/usr/bin/env python3
"""Build cleaned ADNI No-MCI left smooth longitudinal manifests.
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Build ADNI No-MCI left smooth manifest, pairs, and triplets."
    )
    parser.add_argument(
        "--split-dir",
        type=Path,
        default=repo_root / "examples/splits/splits_left_hippocampus_ADNI_smooth_No_MCI",
    )
    parser.add_argument(
        "--metadata-csv",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=adni_root
        / "left_hippocampus_correspondence/minimal_smooth_scaled_obj_files",
    )
    parser.add_argument(
        "--sdf-dir",
        type=Path,
        default=adni_root
        / "left_hippocampus_correspondence/sdf_data_smooth/SdfSamples/minimal_smooth_scaled_obj_files",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
...
def main() -> None:
    ...
    write_csv(metadata_dir / "adni_no_mci_left_smooth_master.csv", master_rows, MASTER_FIELDS)
    write_csv(
        metadata_dir / "adni_no_mci_left_smooth_clean.csv",
        [clean_projection(row) for row in clean_rows_list],
        CLEAN_FIELDS,
    )
    write_json(metadata_dir / "adni_no_mci_left_smooth_subject_trajectories.json", trajectories)
