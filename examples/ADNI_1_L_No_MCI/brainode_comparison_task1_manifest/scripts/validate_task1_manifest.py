...
def validate_required_files(root: Path) -> Tuple[List[Path], List[Path]]:
    required = [
        root / "metadata/adni_no_mci_left_smooth_master.csv",
        root / "metadata/adni_no_mci_left_smooth_clean.csv",
        root / "metadata/adni_no_mci_left_smooth_subject_trajectories.json",
...
def main() -> None:
    ...
    master = read_csv(metadata_dir / "adni_no_mci_left_smooth_master.csv")
    clean = read_csv(metadata_dir / "adni_no_mci_left_smooth_clean.csv")
    pairs_all = read_csv(root / "pairs/pairs_all.csv")
    triplets_all = read_csv(root / "triplets/triplets_all.csv")
    trajectories = read_json(metadata_dir / "adni_no_mci_left_smooth_subject_trajectories.json")
