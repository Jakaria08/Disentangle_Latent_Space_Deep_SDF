#!/usr/bin/env python3
"""Train volume-aware exact-coboundary V2 with a frozen 128-D decoder."""
...
for path in (Path(__file__).resolve().parent, AUGUST_SCRIPTS):
...
def validate_config(config: dict[str, Any]) -> None:
    if config.get("method") != "volume_exact_coboundary_c4_v2":
        raise ValueError("Method must be volume_exact_coboundary_c4_v2")
    if config.get("representation") not in {"pca128", "spiralnet128", "adaptive128"}:
        raise ValueError("Unknown representation")
    model = config.get("model", {})
    if int(model.get("latent_dim", -1)) != C.LATENT_DIM or model.get("variant") != "exact_volume_axis_group_coboundary_v2":
        raise ValueError("Wrong V2 model contract")
    if float(model.get("dropout", 0.0)) != 0.0:
        raise ValueError("Dropout is forbidden inside analytically inverted maps")
    if bool(model.get("ode_used", True)) or bool(model.get("attention_used", True)):
        raise ValueError("V2 uses neither ODE integration nor attention")
    required_loss = {
        "real_latent_weight", "real_vertex_weight", "sequence_latent_weight", "sequence_vertex_weight",
        "volume_weight", "rate_weight", "slope_weight", "group_rate_weight", "disease_gap_weight",
        "axis_volume_weight", "axis_rate_weight", "axis_group_rate_weight",
        "axis_disease_gap_weight", "axis_group_order_weight",
    }
    missing = required_loss.difference(config.get("loss", {}))
    if missing:
        raise KeyError(f"V2 loss is missing {sorted(missing)}")
    selection_required = {
        "all_pair_shape_weight", "volume_weight", "rate_weight", "trend_weight",
        "diagnosis_gap_weight", "latent_nochange_tolerance", "coordinate_nochange_tolerance",
        "max_relative_semigroup_defect", "max_relative_inverse_defect",
        "implementation_audit_max_relative_defect",
    }
    missing = selection_required.difference(config.get("selection", {}))
    if missing:
        raise KeyError(f"V2 selection is missing {sorted(missing)}")
    fraction = float(config["training"]["long_horizon_fraction"])
    if not 0.0 <= fraction < 1.0:
        raise ValueError("long_horizon_fraction must be in [0,1)")
    contract = config.get("scientific_contract", {})
    if not all(bool(contract.get(key)) for key in (
        "coboundary_used", "structural_exactness", "train_only_volume_axis", "volume_aware_selection"
    )):
        raise ValueError("Incomplete V2 scientific contract")


def mixed_pair_indices(
    rows: list[C.PairRow],
    ...
    long_count = int(round(int(samples) * float(long_horizon_fraction)))
    ordinary_count = int(samples) - long_count
    ordinary = list(O.balanced_pair_sampler(rows, seed, epoch, ordinary_count))
    pair_index = {(row.source, row.target): index for index, row in enumerate(rows)}
    first_last = C.first_last_pairs(archive)
    first_last_index = {row.source: pair_index[(row.source, row.target)] for row in first_last}
    balanced_starts = O.balanced_sequence_starts(archive, seed + 11_003, epoch)
    long_indices = [first_last_index[balanced_starts[index % len(balanced_starts)]] for index in range(long_count)]
    combined = ordinary + long_indices
    generator = torch.Generator().manual_seed(int(seed) + 193_939 * int(epoch))
    order = torch.randperm(len(combined), generator=generator).tolist()
    return [int(combined[index]) for index in order]


def validation(
    flow: ExactVolumeCoboundaryFlow,
    ...
    return {
        "score": score, "feasible": feasible, "ratios": ratios,
        "all_pairs": all_metrics, "first_last": first_last_metrics, "defects": defects,
    }
...
def checkpoint_payload(
    ...
    return {
        "epoch": epoch, "flow_state_dict": flow.state_dict(),
        "optimizer_state_dict": optimizer.state_dict(), "scheduler_state_dict": scheduler.state_dict(),
        "config": config, "statistics": statistics, "validation": current_validation,
        ...
        "model_variant": flow.variant, "coboundary_used": True, "structural_exactness": True,
        "train_only_volume_axis": True, "decoder_frozen": True,
...
def main() -> int:
    ...
    volume_axis = O.fit_train_volume_axis(
        train_values, train_archive, float(config["model"]["volume_axis_ridge_alpha"])
    )
    if float(volume_axis["subject_balanced_weighted_r2"]) < float(config["model"]["minimum_volume_axis_r2"]):
        raise RuntimeError(f"Train-only decoded-volume axis is too weak: R2={volume_axis['subject_balanced_weighted_r2']}")
    statistics["volume_axis"] = volume_axis
    flow = build_flow(config, volume_axis["coefficient"]).to(device)
    raw_val = C.cached_vertices("val", registry)

    print("=" * 100)
    print(f"volume exact coboundary V2 | {representation} | device={device} | parameters={C.parameter_count(flow)}")
    print("Phi=F_t o F_s^-1; explicit train-only log-volume potential; shape coupling volume-orthogonal")
    print("decoder=frozen+differentiable; long-horizon sampler=yes; test loaded=no")
    axis_summary = {key: value for key, value in volume_axis.items() if key != "coefficient"}
    statistics_summary = dict(statistics)
    statistics_summary["volume_axis"] = axis_summary
    print(json.dumps({"volume_axis": axis_summary, "training_statistics": statistics_summary}, sort_keys=True))

    initial_indices = mixed_pair_indices(
        train_pairs, train_archive, seed, 0, max(int(training["batch_size"]), 4),
        float(training["long_horizon_fraction"]),
    )
    initial_loader = DataLoader(
        PairDataset(train_pairs), batch_size=int(training["batch_size"]), sampler=initial_indices,
    ...
    flow.zero_grad(set_to_none=True)
    pair["axis_volume"].backward(retain_graph=True)
    volume_head_gradient = gradient_norm(flow.volume_head)
    if not math.isfinite(volume_head_gradient) or volume_head_gradient <= 0.0:
        raise RuntimeError("Explicit volume-potential gradient audit failed")
    flow.zero_grad(set_to_none=True)
    ...
    if not torch.isfinite(total) or not math.isfinite(total_gradient) or total_gradient <= 0.0:
        raise RuntimeError("V2 total-loss gradient audit failed")
    flow.zero_grad(set_to_none=True)
    flow.eval()
    baseline_validation = validation(
        flow, geometry, val_values, val_archive, val_pairs, raw_val, statistics, config, smoke_limit
    )
    maximum_defect = max(baseline_validation["defects"].values())
    if maximum_defect > float(config["selection"]["implementation_audit_max_relative_defect"]):
        raise RuntimeError(f"Exact-coboundary algebra audit failed: {maximum_defect:.3e}")
    if args.dry_run:
        print("DRY RUN PASSED — train-only volume axis, exact algebra, gradients, and validation; no files written.")
        print(json.dumps({
            ...
            "volume_head_gradient_l2_norm": volume_head_gradient,
    ...
    method_dir = "volume_exact_coboundary_c4_v2"
    output_dir = C.output_root(registry) / "training" / representation / method_dir / run_name
    ...
    C.atomic_json(output_dir / "resolved_config.json", {
        ...
        "seed": seed, "epochs": epochs, "smoke": bool(args.smoke), "test_data_loaded": False,
    })
    ...
    C.atomic_json(output_dir / "run_contract.json", {
        ...
        "volume_chart": "train-only subject-balanced ridge direction of frozen-decoder log-volume",
        "shape_chart": "127-D volume-orthogonal affine coupling",
        ...
        "composition_exact": True, "volume_aware_selection": True,
        "decoder_frozen": True, "decoder_in_optimizer_loss": True, "test_data_loaded": False,
    })
    ...
    for epoch in range(start_epoch, epochs + 1):
        flow.train()
        indices = mixed_pair_indices(
            train_pairs, train_archive, seed, epoch, samples_per_epoch,
            float(training["long_horizon_fraction"]),
        )
        loader = DataLoader(
            PairDataset(train_pairs), batch_size=int(training["batch_size"]), sampler=indices,
            collate_fn=C.collate_pairs, num_workers=0,
        )
        ...
        for step, current_raw in enumerate(loader, start=1):
            ...
            if not torch.isfinite(loss):
                raise RuntimeError(f"Non-finite V2 loss at epoch {epoch}, batch {step}")
            loss.backward()
        ...
        ratios = current_validation["ratios"]
        row = {
        ...
        C.atomic_json(output_dir / "training_status.json", {
            ...
            "current_cn_predicted_signed_rate": ratios["first_last_cn_predicted_signed_rate"],
            "current_cn_observed_signed_rate": ratios["first_last_cn_observed_signed_rate"],
            "current_ad_predicted_signed_rate": ratios["first_last_ad_predicted_signed_rate"],
            "current_ad_observed_signed_rate": ratios["first_last_ad_observed_signed_rate"],
            "current_diagnosis_gap_error_ratio": ratios["diagnosis_gap_error_ratio"],
            "coboundary_used": True, "structural_exactness": True,
            "train_only_volume_axis": True, "test_data_loaded": False,
        })
        ...
        print(
            ...
            f"CN={ratios['first_last_cn_predicted_signed_rate']:.5f}/obs{ratios['first_last_cn_observed_signed_rate']:.5f} "
            f"AD={ratios['first_last_ad_predicted_signed_rate']:.5f}/obs{ratios['first_last_ad_observed_signed_rate']:.5f} "
