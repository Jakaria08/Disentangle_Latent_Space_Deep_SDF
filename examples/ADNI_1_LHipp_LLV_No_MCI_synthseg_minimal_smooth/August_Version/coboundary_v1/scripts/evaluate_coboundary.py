#!/usr/bin/env python3
"""Unified validation/test evaluator for exact 128-D coboundary runs."""
...
from coboundary_model import build_flow  # noqa: E402
...
def reverse_rows(rows: list[C.PairRow]) -> list[C.PairRow]:
    return [
        C.PairRow(row.target, row.source, row.intermediate, row.subject, row.diagnosis,
                  f"backward_{row.pair_type}", row.delta_years)
        for row in rows
    ]


def limited_pairs(rows: list[C.PairRow], limit: int | None, first_last: bool = False) -> list[C.PairRow]:
    return O.balanced_first_last_subset(rows, limit) if first_last else O.balanced_subset(rows, limit)
...
def sequence_metrics(
    ...
    for subject_index in range(subject_count):
        ...
        context = values["context"][first:first + 1]
        context_age = values["context_age"][first:first + 1]
        count = len(z) - 1
        direct = transport.transport(
            z[:1].expand(count, -1), age[:1].expand(count), age[1:], label.expand(count),
        ...
        for index in range(1, len(z)):
            current = transport.transport(
                current, age[index - 1:index], age[index:index + 1], label, context, context_age
            )
        ...
        raw_target = torch.from_numpy(
            np.asarray(raw_vertices_mm[first + 1:last], dtype=np.float32).copy()
        ).to(z.device)
        ...
        rollout_backward = []
        current_backward, previous_age = z[-1:], age[-1:]
        for index in range(len(z) - 2, -1, -1):
            current_backward = transport.transport(
                current_backward, previous_age, age[index:index + 1], label, context, context_age
            )
...
def main() -> int:
    ...
    if config.get("method") != "exact_coboundary_c4":
        raise ValueError("Evaluator only accepts exact_coboundary_c4 runs")
    ...
    if not bool(checkpoint.get("coboundary_used")) or not bool(checkpoint.get("structural_exactness")):
        raise ValueError("Checkpoint does not declare exact coboundary semantics")
    ...
    transport = build_flow(config).to(device)
    transport.load_state_dict(checkpoint["flow_state_dict"], strict=True)
    transport.eval()
    forward = limited_pairs(C.load_pairs(args.split, archive, registry), args.max_pairs)
    first_last = limited_pairs(C.first_last_pairs(archive), args.max_pairs, first_last=True)
    backward, first_last_backward = reverse_rows(forward), reverse_rows(first_last)
    ...
    max_defect = max(
        defects["relative_semigroup_defect_mean"], defects["relative_inverse_defect_mean"],
        defects["relative_identity_defect_mean"],
    )
    audit_threshold = float(config["selection"]["implementation_audit_max_relative_defect"])
    if max_defect > audit_threshold:
        raise RuntimeError(f"Exact-coboundary audit failed: {max_defect:.3e} > {audit_threshold:.3e}")
    ...
    report = {
        ...
        "representation_floor": floor, "pair_metrics": pair_metrics,
        "sequence_metrics": sequences, "consistency_defects": defects,
        "subject_bootstrap": bootstrap, "structural_audit_passed": True,
