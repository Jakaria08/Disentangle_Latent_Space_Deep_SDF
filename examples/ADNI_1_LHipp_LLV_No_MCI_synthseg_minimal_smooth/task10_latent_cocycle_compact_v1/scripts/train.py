#!/usr/bin/env python3
"""Train one latent-preserving compact-objective job."""
...
def main() -> int:
    ...
    if not args.resume:
        C.atomic_json(
            output / "resolved_config.json",
            {
                ...
                "objective": objective,
                "top_level_loss_count": int(config["loss_count"]),
                "active_leaf_count": int(config["active_leaf_count"]),
                "test_data_loaded": False,
            },
        )
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        print(
            f"[{representation}/{arm}/s{seed}] epoch {epoch:03d}/{epochs} "
    ...
    summary = {
        ...
        "active_leaf_count": int(config["active_leaf_count"]),
        "learning_rate_profile": str(training["learning_rate_profile"]),
        "seed": seed,
