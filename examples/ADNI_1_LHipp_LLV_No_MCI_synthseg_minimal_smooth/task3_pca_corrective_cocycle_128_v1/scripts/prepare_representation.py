#!/usr/bin/env python3
"""Materialize the safe PCA-corrective 128-D longitudinal archive.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--representations", nargs="+", default=["pca_corrective128"])
...
def dry_run(registry: dict[str, Any], names: list[str], device: torch.device, batch_size: int) -> int:
    ...
    for name in names:
        ...
        if spec["kind"] == "pca":
            ...
            raw = C.corrective_codes(target, contract)
            mean = contract.coefficient_mean
            std = contract.coefficient_std
            archive = common_arrays(source) | {
                "visit_latent_raw_128": raw,
                "visit_latent_standardized_128": ((raw - mean) / std).astype(np.float32),
                "train_latent_mean_128": mean,
...
def main() -> int:
    ...
    for name in names:
        ...
        if spec["kind"] == "pca":
            ...
            model, payload, contract = C.load_corrective_model(device, registry)
            for split in C.SPLITS:
                raw_by_split[split] = C.corrective_codes(vertices[split], contract)
        else:
            raise ValueError(f"Unsupported representation kind: {spec['kind']!r}")
