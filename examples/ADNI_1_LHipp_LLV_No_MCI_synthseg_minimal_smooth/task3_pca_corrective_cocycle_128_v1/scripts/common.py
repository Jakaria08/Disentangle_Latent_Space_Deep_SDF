#!/usr/bin/env python3
"""Shared contracts for the isolated PCA-corrective 128-D cocycle experiment.
...
CORRECTIVE_TASK_ROOT = TASK_ROOT.parent / "task_pca_corrective_deformation_v1"
CORRECTIVE_SOURCE_SCRIPTS = TASK_ROOT.parent / "task_spiral_ae_v1" / "scripts"
...
def load_registry(path: str | Path = REGISTRY_PATH) -> dict[str, Any]:
    ...
    required = {"pca128", "spiralnet128", "adaptive128", "pca_corrective128"}
    if not isinstance(representations, dict) or set(representations) != required:
        raise ValueError(f"Registry must define exactly {sorted(required)}")
    return registry
...
def output_root(registry: dict[str, Any] | None = None) -> Path:
    registry = load_registry() if registry is None else registry
    return require_bulk_path(registry["output_root"], "task output root")
...
def prepared_archive_path(representation: str, split: str, registry: dict[str, Any] | None = None) -> Path:
    ...
    if spec.get("archive_root"):
        return resolve_path(spec["archive_root"]) / f"{split}_subject_sequences_128.npz"
...
@dataclass(frozen=True)
class CorrectivePCAContract:
    """Minimal contract required by the frozen PCA-corrective Spiral decoder."""
    ...
    def summary(self) -> dict[str, Any]:
        return {
            ...
            "coordinate_scale_mm": float(self.coordinate_scale_mm),
            "topology_hash": self.topology_hash,
            "mean_sha256": self.mean_sha256,
            "components_sha256": self.components_sha256,
            "split_counts": {
                split: sum(str(row["split"]) == split for row in self.rows)
                for split in SPLITS
            },
        }


def _corrective_source_module():
    if str(CORRECTIVE_SOURCE_SCRIPTS) not in sys.path:
        sys.path.insert(0, str(CORRECTIVE_SOURCE_SCRIPTS))
    import spiral_common as source_sc

    return source_sc


def load_corrective_contract(
    registry: dict[str, Any] | None = None,
) -> CorrectivePCAContract:
    """Rebuild the exact train-only PCA contract pinned by the corrective checkpoint."""
    registry = load_registry() if registry is None else registry
    spec = representation_spec("pca_corrective128", registry)
    mean_path = resolve_path(spec["pca_mean"])
    components_path = resolve_path(spec["pca_components"])
    if sha256(mean_path) != spec["pca_mean_sha256"]:
        raise ValueError("Corrective PCA mean hash mismatch")
    if sha256(components_path) != spec["pca_components_sha256"]:
        raise ValueError("Corrective PCA component hash mismatch")
    mean = np.load(mean_path, allow_pickle=False).astype(np.float32).reshape(-1)
    components = np.load(components_path, allow_pickle=False).astype(np.float32)
    if components.shape != (LATENT_DIM, mean.size):
        raise ValueError(f"Invalid corrective PCA arrays: mean={mean.shape}, components={components.shape}")
    gram = components.astype(np.float64) @ components.astype(np.float64).T
    if float(np.max(np.abs(gram - np.eye(LATENT_DIM)))) > 5.0e-4:
        raise ValueError("Corrective PCA components are not orthonormal")

    source_sc = _corrective_source_module()
    rows = source_sc.read_manifest()
    for split in SPLITS:
        expected = load_source_archive(split, registry)["visit_scan_ids"].astype(str)
        observed = np.asarray(
            [row["scan_id"] for row in rows if str(row["split"]) == split], dtype=str
        )
        if not np.array_equal(observed, expected):
            raise ValueError(f"Corrective/source scan order mismatch for {split}")

    train = np.asarray(cached_vertices("train", registry, mmap=True), dtype=np.float32)
    flat = train.reshape(len(train), -1)
    coefficients = (flat - mean) @ components.T
    reconstruction = coefficients @ components + mean
    residual = flat - reconstruction
    coefficient_mean = coefficients.mean(axis=0).astype(np.float32)
    coefficient_std = np.maximum(coefficients.std(axis=0), np.float32(1.0e-8)).astype(np.float32)
    residual_rms = float(np.sqrt(np.mean(np.square(residual, dtype=np.float64))))
    coordinate_scale = float(np.sqrt(np.mean(np.square(flat - mean, dtype=np.float64))))
    hashes = {str(row["correspondence_topology_hash"]) for row in rows}
    if len(hashes) != 1:
        raise ValueError(f"Expected one correspondence topology, found {len(hashes)}")
    faces_root = resolve_path(registry["representations"]["pca128"]["pca_model_root"])
    faces = np.load(faces_root / "faces.npy", allow_pickle=False).astype(np.int64)
    return CorrectivePCAContract(
        mean=mean,
        ...
        faces=faces,
        rows=rows,
        topology_hash=next(iter(hashes)),
        mean_sha256=sha256(mean_path),
        components_sha256=sha256(components_path),
    )


def corrective_codes(vertices_mm: np.ndarray, contract: CorrectivePCAContract) -> np.ndarray:
    vertices = np.asarray(vertices_mm, dtype=np.float32)
    flat = vertices.reshape(len(vertices), -1)
    result = (flat - contract.mean) @ contract.components.T
    if result.shape != (len(vertices), LATENT_DIM) or not np.isfinite(result).all():
        raise RuntimeError(f"Invalid corrective coefficient array: {result.shape}")
    return result.astype(np.float32)


def load_corrective_model(
    device: torch.device,
    registry: dict[str, Any] | None = None,
) -> tuple[nn.Module, dict[str, Any], CorrectivePCAContract]:
    registry = load_registry() if registry is None else registry
    spec = representation_spec("pca_corrective128", registry)
    checkpoint_path = resolve_path(spec["checkpoint"])
    if sha256(checkpoint_path) != spec["checkpoint_sha256"]:
        raise ValueError("PCA-corrective checkpoint hash mismatch")
    checkpoint = torch.load(checkpoint_path, map_location="cpu", weights_only=False)
    if int(checkpoint.get("best_epoch", -1)) != int(spec["checkpoint_best_epoch"]):
        raise ValueError("Unexpected PCA-corrective best epoch")
    contract = load_corrective_contract(registry)
    ...
    for key in ("latent_dim", "n_vertices", "features", "topology_hash", "mean_sha256", "components_sha256"):
        if saved_contract.get(key) != contract.summary().get(key):
            raise ValueError(f"Corrective checkpoint contract mismatch for {key}")
    from corrective_model import build_model

    model = build_model(checkpoint["config"], contract, device)
...
class FrozenCorrectiveGeometry(FrozenGeometry):
    """Map train-standardized corrective-PCA coefficients to corrected millimetre meshes."""

    def __init__(
        self,
        model: nn.Module,
        contract: CorrectivePCAContract,
        latent_mean: np.ndarray,
        latent_std: np.ndarray,
    ) -> None:
        super().__init__(contract.faces)
        self.decoder = model
        self.register_buffer("latent_mean", torch.from_numpy(latent_mean.astype(np.float32)).view(1, -1))
        self.register_buffer(
            "latent_std",
            torch.from_numpy(np.maximum(latent_std, 1.0e-8).astype(np.float32)).view(1, -1),
        )

    def raw_coefficients(self, standardized_latent: torch.Tensor) -> torch.Tensor:
        return standardized_latent * self.latent_std + self.latent_mean

    def details(self, standardized_latent: torch.Tensor) -> dict[str, torch.Tensor]:
        return self.decoder.decode_with_details(self.raw_coefficients(standardized_latent))

    def vertices(self, standardized_latent: torch.Tensor) -> torch.Tensor:
        return self.details(standardized_latent)["prediction"]

    def pca_vertices(self, standardized_latent: torch.Tensor) -> torch.Tensor:
        return self.details(standardized_latent)["pca_mesh"]
...
def build_geometry(
    ...
    elif spec["kind"] in {"spiral_ae", "adaptive_ae"}:
    ...
    elif spec["kind"] == "pca_corrective":
        model, _checkpoint, contract = load_corrective_model(device, registry)
        geometry = FrozenCorrectiveGeometry(model, contract, latent_mean, latent_std)
    else:
        raise ValueError(f"Unsupported representation kind: {spec['kind']!r}")
