#!/usr/bin/env python3
"""Train direct, non-ODE, non-coboundary C4 with a frozen decoder loss."""
...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if config.get("representation") != "pca_corrective128":
    ...
    if bool(model.get("ode_used", True)) or bool(model.get("attention_used", True)):
        raise ValueError("Direct C4 cannot use ODE integration or attention")
    loss = config.get("loss", {})
    if "coboundary_weight" not in loss or float(loss["coboundary_weight"]) != 0.0:
        raise ValueError("Coboundary must be explicitly present and exactly zero")
    if any("coboundary" in key and float(value) != 0.0 for key, value in loss.items()):
        raise ValueError("All coboundary weights must be zero")
    required = {
        "real_latent_weight",
        "real_vertex_weight",
        "observed_semigroup_weight",
        "virtual_semigroup_weight",
        "inverse_weight",
        "sequence_latent_weight",
        "sequence_vertex_weight",
        "sequence_semigroup_weight",
        "volume_weight",
        "rate_weight",
        "slope_weight",
        "group_rate_weight",
    ...
    if not required.issubset(loss):
        raise KeyError(f"C4 loss is missing {sorted(required.difference(loss))}")


def zero_sequence(reference: torch.Tensor) -> dict[str, torch.Tensor]:
    zero = reference * 0.0
    return {"sequence_latent": zero, "sequence_vertex": zero, "sequence_semigroup": zero, "slope": zero}
...
def main() -> int:
    ...
    # Test is deliberately neither opened nor validated in this trainer.
    ...
    print("=" * 96)
    print(f"direct C4 | {representation} | device={device} | parameters={C.parameter_count(flow)}")
    print("transport=z+(t-s)*velocity; ODE=no; attention=no; coboundary=no")
    print("decoder=frozen but differentiable to predicted latent; test archive loaded=no")
    print(json.dumps(statistics, sort_keys=True))

    initial_sampler = O.balanced_pair_sampler(
        train_pairs,
        seed,
        0,
        max(int(training["batch_size"]), 4),
    )
    ...
    # Explicitly prove that the frozen-decoder reconstruction term trains the flow.
    flow.zero_grad(set_to_none=True)
    pair["real_vertex"].backward(retain_graph=True)
    reconstruction_gradient = math.sqrt(sum(
    ...
    if not torch.isfinite(total) or not math.isfinite(total_gradient) or total_gradient <= 0.0:
        raise RuntimeError("C4 total-loss gradient audit failed")
    flow.zero_grad(set_to_none=True)
    flow.eval()
    ...
    if args.dry_run:
        print("DRY RUN PASSED — C4 finite gradients, decoder reconstruction routing, validation, and no-coboundary guards; no files written.")
        print(json.dumps({
    ...
    output_dir = C.output_root(registry) / "training" / representation / "direct_c4" / run_name
    ...
    C.atomic_json(output_dir / "run_contract.json", {
        "transport": "Phi(z,s,t,d)=z+(t-s)*phi(z,s,t,d)",
        "model_variant": "direct",
        "ode_used": False,
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for step, raw in enumerate(loader, start=1):
            ...
            sequence = O.sequence_terms(
                ...
                train_values,
                train_archive,
                sequence_starts[(step - 1) % len(sequence_starts)],
                statistics,
            )
        ...
        print(f"epoch {epoch:03d}/{epochs} loss={totals['total']/max(batches,1):.6f} val={score:.6f} feasible={feasible} best={best_score:.6f}@{best_epoch}", flush=True)
