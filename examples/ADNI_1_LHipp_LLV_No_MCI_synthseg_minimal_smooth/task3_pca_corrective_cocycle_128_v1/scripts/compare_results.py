#!/usr/bin/env python3
"""Collect the four completed 128-D Direct-C4 runs into the prior comparison tables."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--run", action="append", type=Path, required=True, help="Repeat for the four selected Direct-C4 run directories.")
...
def main() -> int:
    ...
    expected = {
        (representation, "direct_c4")
        for representation in ("pca128", "spiralnet128", "adaptive128", "pca_corrective128")
    }
    if not args.allow_partial and seen != expected:
        raise ValueError(f"Expected four Direct-C4 representations; missing={sorted(expected-seen)}, extra={sorted(seen-expected)}")
    ...
    output = C.require_bulk_path(args.output, "comparison output")
