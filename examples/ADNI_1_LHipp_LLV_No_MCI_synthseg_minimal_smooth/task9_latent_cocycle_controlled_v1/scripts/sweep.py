#!/usr/bin/env python3
"""Dispatch the preregistered matched latent-only experiment."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--loss-set", default=None, help="comma-separated loss sets")
...
def selected_jobs(args: argparse.Namespace) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    filters = {
        "loss_set": set(args.loss_set.split(",")) if args.loss_set else None,
...
def main() -> int:
    ...
    if args.dry_run:
        for job in jobs:
            print(
                f"{job['id']:38s} batch={job['training']['batch_size']:3d} "
                f"epochs={job['training']['epochs']} steps/epoch={job['training']['steps_per_epoch']}"
            )
        return 0
