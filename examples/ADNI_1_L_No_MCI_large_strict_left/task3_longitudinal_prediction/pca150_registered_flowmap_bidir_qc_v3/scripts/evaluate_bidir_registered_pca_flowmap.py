...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--run-name", default="bidir_e04_k16_full_volume_local_seed42")
...
def predict_composed_observed(
    ...
    for index in range(record.source_index, record.target_index, step):
        ...
        target_norm = float(archive["visit_continuous_age_norm"][next_index])
        source_years = float(archive["visit_continuous_age_years"][index])
        target_years = float(archive["visit_continuous_age_years"][next_index])
...
def prediction_metrics(
    *,
    ...
    delta_years = float(record.delta_years)
    if abs(delta_years) < 1.0e-6:
        delta_years = 1.0e-6
...
def summarize_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    group_specs = [
        ...
        ("split_direction_transport", ["split", "direction", "transport_method"]),
...
def build_volume_trends(rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    baseline_rows = [
        row
        for row in rows
        if int(row["source_visit_order"]) == 0 and row.get("direction") == "forward"
    ]
...
def build_ood_gt_summary(rows: list[dict[str, Any]], min_gap_years: float) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ood_rows = [
        row
        for row in rows
        if row.get("direction") == "forward" and float(row["gap_years"]) >= float(min_gap_years)
    ]
...
def build_cycle_rows(
    *,
    ...
    archive = load_split_archive(split)
    latents = pca_latents(archive, int(args.components))
    records = build_pair_records(archive, include_backward=False)
    if int(args.max_pairs_per_split) > 0:
        records = records[: int(args.max_pairs_per_split)]
    ...
    rows: list[dict[str, Any]] = []
    for record in records:
        source = torch.from_numpy(latents[record.source_index].astype(np.float32)).view(1, -1).to(device)
        condition = torch.tensor([float(record.label_ad)], dtype=torch.float32, device=device)
        forward = model.transport(
            source,
            torch.tensor([record.source_age_norm], dtype=torch.float32, device=device),
            torch.tensor([record.target_age_norm], dtype=torch.float32, device=device),
            torch.tensor([record.source_age_years], dtype=torch.float32, device=device),
        ...
        source_vertices = (
            (source @ component_tensor + mean_tensor).reshape(1, -1, 3)
        )
        cycle_vertices = (
            (cycle @ component_tensor + mean_tensor).reshape(1, -1, 3)
        )
        vertex_error = torch.linalg.norm(cycle_vertices - source_vertices, dim=2).mean()
        rows.append(
            {
                "split": split,
                "subject_id": record.subject_id,
                ...
                "target_visit_order": record.target_visit_order,
                "gap_years": record.abs_gap_years,
                "cycle_pca_mse": float(torch.mean((cycle - source) ** 2).detach().cpu().item()),
                "cycle_vertex_euclidean": float(vertex_error.detach().cpu().item()),
            }
        )
    return rows


def summarize_cycle_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    groups: dict[tuple[str, str], list[dict[str, Any]]] = {}
    for row in rows:
        groups.setdefault((row["split"], row["diagnosis"]), []).append(row)
    summary: list[dict[str, Any]] = []
    for (split, diagnosis), group_rows in groups.items():
        summary.append(
            {
                "split": split,
                "diagnosis": diagnosis,
                "rows": len(group_rows),
                "cycle_pca_mse_mean": finite_mean(float(row["cycle_pca_mse"]) for row in group_rows),
                "cycle_vertex_euclidean_mean": finite_mean(
                    float(row["cycle_vertex_euclidean"]) for row in group_rows
                ),
            }
        )
    return summary
...
def build_sequence_starts_for_eval(archive: dict[str, np.ndarray], min_length: int = 3) -> list[int]:
    starts: list[int] = []
    ...
    for subject_index in range(len(offsets) - 1):
        ...
        if subject_end - subject_start >= int(min_length):
            starts.append(subject_start)
    return starts


@torch.no_grad()
def evaluate_sequence_rows(
    *,
    model: RegisteredPCAFlowMap,
    split: str,
    args: argparse.Namespace,
    ...
    archive = load_split_archive(split)
    latents = pca_latents(archive, int(args.components))
    starts = build_sequence_starts_for_eval(archive)
    if int(args.max_pairs_per_split) > 0:
        starts = starts[: int(args.max_pairs_per_split)]
    mean_tensor = torch.from_numpy(mean_flat).to(device)
    component_tensor = torch.from_numpy(components).to(device)
    rows: list[dict[str, Any]] = []
    for start in starts:
        ...
        source = torch.from_numpy(latents[start].astype(np.float32)).view(1, -1).to(device)
        current = source
        direct_errors: list[float] = []
        composed_errors: list[float] = []
        for index in range(start + 1, end):
            target = torch.from_numpy(latents[index].astype(np.float32)).view(1, -1).to(device)
            direct = model.transport(
                source,
                torch.tensor([float(archive["visit_continuous_age_norm"][start])], dtype=torch.float32, device=device),
                torch.tensor([float(archive["visit_continuous_age_norm"][index])], dtype=torch.float32, device=device),
                torch.tensor([float(archive["visit_continuous_age_years"][start])], dtype=torch.float32, device=device),
                torch.tensor([float(archive["visit_continuous_age_years"][index])], dtype=torch.float32, device=device),
                condition,
            )
            current = model.transport(
                current,
                torch.tensor([float(archive["visit_continuous_age_norm"][index - 1])], dtype=torch.float32, device=device),
                torch.tensor([float(archive["visit_continuous_age_norm"][index])], dtype=torch.float32, device=device),
                torch.tensor([float(archive["visit_continuous_age_years"][index - 1])], dtype=torch.float32, device=device),
                torch.tensor([float(archive["visit_continuous_age_years"][index])], dtype=torch.float32, device=device),
                condition,
            )
            target_vertices = (target @ component_tensor + mean_tensor).reshape(1, -1, 3)
            direct_vertices = (direct @ component_tensor + mean_tensor).reshape(1, -1, 3)
            composed_vertices = (current @ component_tensor + mean_tensor).reshape(1, -1, 3)
            direct_errors.append(
                float(torch.linalg.norm(direct_vertices - target_vertices, dim=2).mean().detach().cpu().item())
            )
            composed_errors.append(
                float(torch.linalg.norm(composed_vertices - target_vertices, dim=2).mean().detach().cpu().item())
            )
        if direct_errors:
            rows.append(
                {
                    ...
                    "start_scan_id": str(archive["visit_scan_ids"][start]),
                    "sequence_length": end - int(start),
                    "direct_sequence_vertex_euclidean_mean": finite_mean(direct_errors),
                    "composed_sequence_vertex_euclidean_mean": finite_mean(composed_errors),
                }
            )
    return rows


def summarize_sequence_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    groups: dict[tuple[str, str], list[dict[str, Any]]] = {}
    for row in rows:
        groups.setdefault((row["split"], row["diagnosis"]), []).append(row)
    summary: list[dict[str, Any]] = []
        ...
        summary.append(
            {
                ...
                "diagnosis": diagnosis,
                "rows": len(group_rows),
                "direct_sequence_vertex_euclidean_mean": finite_mean(
                    float(row["direct_sequence_vertex_euclidean_mean"]) for row in group_rows
                ),
                "composed_sequence_vertex_euclidean_mean": finite_mean(
                    float(row["composed_sequence_vertex_euclidean_mean"]) for row in group_rows
                ),
            }
        )
    return summary
...
def main() -> int:
    ...
    forward_rows = [row for row in pair_rows if row.get("direction") == "forward"]
    backward_rows = [row for row in pair_rows if row.get("direction") == "backward"]

    write_csv(output_dir / "registered_flow_per_pair.csv", pair_rows)
    write_csv(output_dir / "forward_per_pair.csv", forward_rows)
    write_csv(output_dir / "backward_per_pair.csv", backward_rows)
    ...
    if bool(args.include_cycle_eval):
        for split in args.splits:
            cycle_rows.extend(
                build_cycle_rows(
                    model=model,
        ...
        cycle_summary_rows = summarize_cycle_rows(cycle_rows)
        write_csv(output_dir / "cycle_consistency.csv", cycle_rows)
        write_csv(output_dir / "cycle_consistency_summary.csv", cycle_summary_rows)

    sequence_rows: list[dict[str, Any]] = []
    sequence_summary_rows: list[dict[str, Any]] = []
    if bool(args.include_sequence_eval):
        for split in args.splits:
            sequence_rows.extend(
                evaluate_sequence_rows(
                    model=model,
                    split=split,
                    args=args,
                    device=device,
        ...
        sequence_summary_rows = summarize_sequence_rows(sequence_rows)
        write_csv(output_dir / "sequence_rollout.csv", sequence_rows)
        write_csv(output_dir / "sequence_rollout_summary.csv", sequence_summary_rows)
    ...
    run = {
        ...
        "cycle_summary_rows": len(cycle_summary_rows),
        "sequence_rows": len(sequence_rows),
        "sequence_summary_rows": len(sequence_summary_rows),
