...
METHOD_ORDER = ("real_observed", "direct_from_base", "base_reconstruction_no_change")
METHOD_LABELS = {
    "real_observed": "real observed",
    "direct_from_base": "prediction",
...
def make_final_delta_comparison_figure(subject_summary: pd.DataFrame) -> go.Figure:
    ...
    for col, split in enumerate(splits, start=1):
        ...
        for method, y_col, color in [
            ...
            ("prediction", "mean_predicted_final_delta_pct", "#1f77b4"),
        ]:
            figure.add_trace(
                go.Bar(
                    x=group["diagnosis"],
...
def make_speed_box_figure(speed_frame: pd.DataFrame) -> go.Figure:
    ...
    usable_methods = ("real_observed", "direct_from_base")
    for col, split in enumerate(splits, start=1):
    ...
    figure.update_layout(
        title="Adjacent-interval volume-change speed: real data vs prediction",
        template="plotly_white",
...
def make_gap_bin_figure(gap_summary: pd.DataFrame) -> go.Figure:
    ...
    figure = make_subplots(
        rows=1,
        cols=2,
        subplot_titles=("SDF improvement by gap", "Beat no-change fraction by gap"),
    )
    for split in ("train", "val", "test"):
        ...
        figure.add_trace(
            go.Scatter(
                x=group["gap_bin"],
                y=group["relative_improvement_pct"],
                mode="lines+markers",
                name=split,
                legendgroup=split,
            ),
    ...
    if (
        ...
        selected = selected_speed_frame.loc[
            selected_speed_frame["transport_method"].isin(
                ("real_observed", "direct_from_base")
            )
        ].copy()
