# Task 1: ADNI With-MCI Left Original Longitudinal Manifest

This folder is an isolated data-contract layer for the BrainODE comparison.
It reads existing ADNI metadata, original full-diagnosis split JSON files,
original meshes, and original SDF samples, then writes cleaned CSV/JSON
artifacts for later tasks.

No existing dataset files, split files, specs, meshes, or SDF samples are edited.

## Inputs

- `examples/splits/splits_left_hippocampus_ADNI/*.json`
...
- `/home/jakaria/ADNI/ADNI_1/adni_processed/left_hippocampus_correspondence/sdf_data/SdfSamples/minimal_scaled_obj_files`

## Outputs

- `metadata/adni_with_mci_left_original_master.csv`
- `metadata/adni_with_mci_left_original_clean.csv`
- `metadata/adni_with_mci_left_original_subject_trajectories.json`
...
`label_dx` stores the full diagnosis encoding:

- `CN -> 0`
- `MCI -> 1`
- `AD -> 2`

`label_ad` is preserved only for binary CN/AD compatibility:
...
```bash
python examples/ADNI_1_L_With_MCI/brainode_comparison_task1_manifest_original/scripts/build_task1_manifest.py
python examples/ADNI_1_L_With_MCI/brainode_comparison_task1_manifest_original/scripts/validate_task1_manifest.py
```
